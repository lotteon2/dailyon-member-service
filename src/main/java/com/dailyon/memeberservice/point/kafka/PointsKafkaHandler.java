...
import com.dailyon.memeberservice.point.kafka.dto.BiddingDTO;
...
public class PointsKafkaHandler {
    ...
    public void usePoints(String message, Acknowledgment ack) {
        ...
        try {
            ...
            if (orderDto.getReferralCode() != null
                    && !member.getCode().equals(orderDto.getReferralCode())) {
                ...
                RawNotificationData rawNotificationData =
                        RawNotificationData.forPointEarnedByReferralCode(fixedPointAmount);
                SQSNotificationDto sqsNotificationDto =
                        SQSNotificationDto.of(refMember.getId(), rawNotificationData);
    ...
    public void addPoints(String message, Acknowledgment ack) {
        ...
        try {
            ...
            Member member =
                    memberRepository
                            .findById(reivewDto.getMemberId())
                            .orElseThrow(() -> new RuntimeException("Member not found"));
    ...
    @KafkaListener(topics = "success-bidding")
    @Transactional
    public void usePointsForBidding(String message, Acknowledgment ack) {
        BiddingDTO biddingDTO = null;
        try {
            biddingDTO = objectMapper.readValue(message, BiddingDTO.class);
            Member member =
                    memberRepository
                            .findById(biddingDTO.getMemberId())
                            .orElseThrow(() -> new RuntimeException("Member not found"));
            PointHistory pointHistory =
                    PointHistory.builder()
                            .member(member)
            ...
            usePointsForBiddingFail(biddingDTO);
        } finally {
            ack.acknowledge();
        }
    }

    public void usePointsForBiddingFail(BiddingDTO biddingDTO) {
        try {
            String data = objectMapper.writeValueAsString(biddingDTO);
            kafkaTemplate.send("cancel-bidding", data);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
